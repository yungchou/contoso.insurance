param($cloudShopUrl, $datasource="CloudShopSQL")
...
if ($datasource -ne "CloudShopSQL"){
    # Replace the database name in the Web.Config
    (Get-Content "C:\inetpub\wwwroot\Web.config").replace('CloudShopSQL', $datasource) | Set-Content "C:\inetpub\wwwroot\Web.config"
}
